...
template <idx_t block_y_size, idx_t temporal_steps, idx_t words_per_thread, typename word_type>
__device__ __forceinline__ void null_own_cells_in(word_type* shared) {
    using c = consts<block_y_size, temporal_steps, words_per_thread>;
...
__global__ void game_of_live_kernel(GridModel<word_type> data) {
    ...
    bool skip_computation = is_beyond_the_edge_of_grid_and_should_not_compute<block_y_size, temporal_steps, words_per_thread>();

    if (skip_computation) {
        null_own_cells_in<block_y_size, temporal_steps, words_per_thread>(next_buffer);
    }
